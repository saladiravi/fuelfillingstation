    ...
    const processedRows = await Promise.all(
      result.rows.map(async (row) => {
        if (row.operatorshift === 'MID-A') {
          ...
          // A's OMR is MID-A's CMR if exists, otherwise previous day's B shift CMR
          const midAQuery = `
            SELECT ps."cmr"
            FROM pump_sales ps
            INNER JOIN attendence a ON ps.attendence_id = a.attendence_id
            WHERE 
              ...
              AND ps.fuel_type = $3;
          `;
          const midAResult = await pool.query(midAQuery, [date, row.bay_side, row.fuel_type]);
    
          if (midAResult.rows.length > 0) {
            row.omr = midAResult.rows[0].cmr;
          } else {
            // If MID-A does not exist, take previous day's B shift CMR
            const prevShiftQuery = `
              SELECT ps."cmr"
              FROM pump_sales ps
              INNER JOIN attendence a ON ps.attendence_id = a.attendence_id
        ...
        `;
          ...
          // B's OMR is MID-B's CMR if exists, otherwise A shift's CMR
          const midBQuery = `
            SELECT ps."cmr"
            FROM pump_sales ps
            INNER JOIN attendence a ON ps.attendence_id = a.attendence_id
            WHERE 
              ...
              AND ps.fuel_type = $3;
          `;
          const midBResult = await pool.query(midBQuery, [date, row.bay_side, row.fuel_type]);
    
          if (midBResult.rows.length > 0) {
            row.omr = midBResult.rows[0].cmr;
          } else {
            // If MID-B does not exist, take A shift's CMR
            const sameDayShiftQuery = `
              SELECT ps."cmr"
              FROM pump_sales ps
              INNER JOIN attendence a ON ps.attendence_id = a.attendence_id

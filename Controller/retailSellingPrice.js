...
exports.addsellingprice=async (req,res)=>{
    try{
      ...
      const sellingprice=await pool.query(`
        INSERT INTO retailsellingprice("date","ms","psd","speed") values ($1,$2,$3,$4) RETURNING *`,
    [date,ms,psd,speed]);
        res.status(200).json({
            statusCode:200,
            message:'price added sucessful',
            sellingprices:sellingprice[0],
            
        })
        if(sellingprice.date==req.date){
